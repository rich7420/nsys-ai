...
def gpu_trace(prof, gpu: int, trim: tuple[int, int]) -> list[dict]:
    ...
    # Metadata
    events.append(dict(name="process_name", ph="M", pid=gpu, args=dict(name=f"GPU {gpu}")))
    for d in range(max_depth + 1):
        events += [
            dict(name="thread_name", ph="M", pid=gpu, tid=10 + d, args=dict(name=f"NVTX Lvl {d}")),
            dict(
                name="thread_sort_index", ph="M", pid=gpu, tid=10 + d, args=dict(sort_index=10 + d)
            ),
        ]
    for s in sorted(streams):
        ...
        events += [
            dict(
                name="thread_name", ph="M", pid=gpu, tid=s, args=dict(name=f"Stream {s} ({label})")
            ),
            dict(name="thread_sort_index", ph="M", pid=gpu, tid=s, args=dict(sort_index=50 + s)),
        ]
    return events
